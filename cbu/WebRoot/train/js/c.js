(function(aa) {
	jQuery.extend({
		ht_getcookie : function(a) {
			...
			return b == -1 ? "" : unescape(document.cookie.substring(b
					+ a.length + 1, (c > b ? c : document.cookie.length)))
		},
		...
		textFocus : function(a) {
			...
			this.each(function() {
				if (!this.setSelectionRange) {
					...
					c = this.value.length;
					a === 0 ? this.setSelectionRange(c, c) : this
	...
	aa.stationFor12306 = {
		...
		city_Bind : function(a) {
			...
			aa.each(a, function(c) {
				if (Q == a[c][2]) {
					b += "<div class='cityline' id='citem_" + c + "' cturn='"
							+ a[c][6] + "'><span class='ralign'><b>" + a[c][1]
							+ "</b></span></div>\n"
				} else {
					b += "<div class='cityline' id='citem_" + c + "' cturn='"
							+ a[c][6] + "'><span class='ralign'>" + a[c][1]
							+ "</span></div>\n"
				}
			});
		...
		city_changeSelectIndex : function(b) {
			...
			if (a == -1) {
				...
				aa.stationFor12306.city_shiftSelect(aa("#citem_"
						+ (X.length - 1)))
			} else {
				if (a == X.length) {
					aa.stationFor12306.city_showlist(ad + 1);
		...
		city_shiftSelect : function(b) {
			if (ag != b) {
				if (ag != 0) {
					aa(ag).removeClass("citylineover").addClass("cityline")
							.css("backgroundColor", "white")
				}
				if (b != 0) {
					try {
						ag = b;
						var c = aa(ag).removeClass("cityline").addClass(
		...
		js : function(c) {
			...
			for (d = 1; d <= 7; d++) {
				if (d == c) {
					...
					if (d > 1 && d < 7) {
						var a = aa.stationFor12306.tHtmlGetCityName(c - 1, -1,
								0);
						if (a > ah) {
				...
				aa(".ac_even").on("mouseover", function() {
					aa.stationFor12306.city_shiftSelectInLi(this)
						...
						function() {
							...
							if (ab) {
								aa.stationFor12306.setStationInCookies(aa(this)
										.text(), aa(this).attr("data"))
							}
							aa("#form_cities2").css("display", "none");
							ap = -1;
							ae = 0;
		...
		tHtmlGetCityName : function(c, d, a) {
			...
			case 1:
				...
				if (au.length > ah) {
					...
					if (b > 1) {
						ai = au.slice(ah * (a), Math.min(ah * (a + 1),
								au.length));
						return ai[d]
					}
				}
			...
			case 2:
				...
				if (av.length > ah) {
					...
					if (b > 1) {
						aj = av.slice(ah * (a), Math.min(ah * (a + 1),
								av.length));
						return aj[d]
					}
				}
			...
			case 3:
				...
				if (aw.length > ah) {
					...
					if (b > 1) {
						al = aw.slice(ah * (a), Math.min(ah * (a + 1),
								aw.length));
						return al[d]
					}
				}
			...
			case 4:
				...
				if (ax.length > ah) {
					...
					if (b > 1) {
						am = ax.slice(ah * (a), Math.min(ah * (a + 1),
								ax.length));
						return am[d]
					}
				}
		...
		showAllCity : function() {
			...
			d = '<div class="com_hotresults" id="thetable" style="width:'
					+ j
			...
			d = d
					+ '<li index="1" method="liHotTab"  onclick="$.stationFor12306.js(1)" id="nav_list1">热门</li><li index="2" method="liHotTab"  onclick="$.stationFor12306.js(2)" id="nav_list2">A－E</li><li index="3" method="liHotTab"  onclick="$.stationFor12306.js(3)" id="nav_list3">F－J</li><li index="4" method="liHotTab"  onclick="$.stationFor12306.js(4)" id="nav_list4">K－O</li><li index="5" method="liHotTab"  onclick="$.stationFor12306.js(5)" id="nav_list5">P－T</li><li index="6" method="liHotTab"  onclick="$.stationFor12306.js(6)" id="nav_list6">U－Z</li></ul>';
			if (ab) {
				...
				if (g > 2) {
					R = true;
					for ( var b = 0; b < g; b++) {
						d += '<li class="ac_even"   title="' + c[b][0]
								+ '" data="' + c[b][1] + '">' + c[b][0]
								+ "</li>"
					}
				}
			...
			for ( var b = 0; b < h; b++) {
				d += '<li class="ac_even' + e + '"   title="'
						+ aa.stationFor12306.tHtmlGetCityName(0, b, 0)[1]
						+ '" data="'
						+ aa.stationFor12306.tHtmlGetCityName(0, b, 0)[2]
						+ '">'
						+ aa.stationFor12306.tHtmlGetCityName(0, b, 0)[1]
						+ "</li>"
			}
			...
			for ( var a = 2; a <= 6; a++) {
				...
				if (k > ah) {
					...
					if (i > 1) {
						d += '<ul  class="popcitylist" style="overflow: auto; max-height: 260px; height: 170px;display:none;" id="ul_list'
								+ a + '">';
						aa.stationFor12306.pageDesigh(i, 0, a)
					}
					aa("#flip_cities2").css("display", "block")
				} else {
					d += '<ul  class="popcitylist" style="overflow: auto; max-height: 260px; height: 191px;display:none;" id="ul_list'
							+ a + '">';
					aa("#flip_cities2").css("display", "none");
					var e = "";
					if (!ab) {
					...
					for ( var b = 0; b < aa.stationFor12306.tHtmlGetCityName(f,
							-1, 0); b++) {
						d += '<li class="ac_even'
								+ e
								+ '"   title="'
								+ aa.stationFor12306.tHtmlGetCityName(f, b, 0)[1]
								+ '" data="'
								+ aa.stationFor12306.tHtmlGetCityName(f, b, 0)[2]
								+ '">'
								+ aa.stationFor12306.tHtmlGetCityName(f, b, 0)[1]
								+ "</li>"
					}
		...
		pageDesigh : function(c, a, b) {
			...
			if (c > 1) {
				...
				T = aa.stationFor12306.tHtmlGetCityName(b - 1, -2, 0).slice(
						ah * (a),
						Math.min(ah * (a + 1), aa.stationFor12306
								.tHtmlGetCityName(b - 1, -2, 0).length));
				var d = "";
				if (!ab) {
					d = " openLi"
				}
				for ( var g = 0; g < T.length; g++) {
					var f = T[g];
					e += '<li class="ac_even' + d + '"   title="' + f[1]
							+ '" data="' + f[2] + '">' + f[1] + "</li>"
				}
				aa("#ul_list" + b).html(e);
				var h = (a == 0) ? "&laquo;&nbsp;上一页"
		...
		filterCity : function(a) {
			...
			if (c.length > 0) {
				aa("#top_cities")
						.html('按"<font color=red>' + a + '</font>"检索：');
				return c
			} else {
		...
		city_showlist : function(a) {
			if (Y.length > 6) {
				...
				var c = (a == 0) ? "<span style='float:left;'>&laquo;&nbsp;向前</span>"
						: "<a style='float:left;cursor:pointer' href='' class='cityflip' onclick='$.stationFor12306.city_showlist("
								+ (a - 1)
								+ ");return false;'>&laquo;&nbsp;向前</a>";
				c += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
				c += (a == b - 1) ? "<span style='float:right;'>向后&nbsp;&raquo;</span>"
						: "<a style='float:right;cursor:pointer' href='' class='cityflip' onclick='$.stationFor12306.city_showlist("
								+ (a + 1)
		...
		SetISPos : function(a) {
			if (P) {
				...
				aa("#form_cities").css("top",
						a.position().top + a.height() + 12);
				aa("#form_cities2").css("left", a.position().left);
				aa("#form_cities2").css("top",
						a.position().top + a.height() + 12)
			}
			var b = a.offset().top;
			var e = aa("#search_div");
		...
		bindEvent : function(c) {
			...
			aa(b).keydown(
					function(d) {
						...
						aa("#flip_cities2").css("display", "none");
						aa(".ac_even").removeClass("ac_over")
								.addClass("ac_odd");
						d = d || window.event;
						if (d.keyCode == 40) {
								...
								if (d.keyCode == 13) {
									...
									if (document.addEventListener) {
										document.addEventListener("keypress",
												aa.stationFor12306.myHandlerFg,
												true)
									} else {
										evt = window.event;
										evt.keyCode = 9
					...
					function() {
						...
						if (ay) {
							...
							if (ap == -1) {
								...
								aa(".ac_even").removeClass("ac_over").addClass(
										"ac_odd");
								aa("#form_cities2").css("display", "block")
							}
						}
				...
				if (!at && !W) {
					...
					function(d) {
						...
						ay = true;
						d = d || window.event;
						if (d.keyCode != 40 && d.keyCode != 38
								&& d.keyCode != 37 && d.keyCode != 39
								&& d.keyCode != 13 && d.keyCode != 9) {
		...
		setStationInCookies : function(c, l) {
			...
			for (; h < e; h++) {
				...
				k += g[h][0] + "#" + g[h][1];
				if (R && ab) {
					d += '<li class="ac_even" onmouseover="$.stationFor12306.city_shiftSelectInLi(this);" onclick="$.stationFor12306.li_click(this);"   title="'
							+ g[h][0]
							+ '" data="'
							+ g[h][1]
							+ '">'
		...
		li_click : function(a) {
			...
			if (ab) {
				aa.stationFor12306.setStationInCookies(aa(a).text(), aa(a)
						.attr("data"))
			}
			aa("#form_cities2").css("display", "none");
			ap = -1;
	...
	$.stopStation = function(d) {
		...
		c.init = function() {
			...
			if (!$("#" + c.options.showDivId)[0]) {
				var a = [];
				var b = -1;
				a[++b] = '<div class="station" style="display:none;" id="'
						+ c.options.showDivId + '"><b></b>';
				a[++b] = '<div class="station-info" id="'
						+ c.options.showTitleId + '"></div>';
				...
				a[++b] = '<div class="station-bd"><table><tbody id="'
						+ c.options.showTableId
						+ '"></tbody></table></div></div>';
			...
			if (c.options.mouseOutClose) {
				$("#" + c.options.showDivId).on("mouseenter", function(f) {
					c.options.mouseOnPanel = 1
						...
						function() {
							c.options.mouseOnPanel = 0;
							$("#" + c.options.showDivId).hide().appendTo(
									$("body:eq(0)"));
...
(function() {
	...
	var aI = true, bA = true, aD = true, ak = "starttime";
	var an = true;
	var ba = [];
	var aU = [];
	var bs = [];
	...
	function am(bR) {
					...
					if (ccSelected.length > 0 || rqChecked.length > 0
						...
						if (train_tour_flag == "fc") {
							var bS = "成功查到"
									+ $("#back_train_date").val()
									+ "的"
									+ bl[0]["queryLeftNewDTO"]["station_train_code"]
									+ "次"
						} else {
							var bS = "成功查到"
									+ $("#train_date").val()
		...
		for ( var bT = 0; bT < bR.length; bT++) {
			...
			if (bR[bT].queryLeftNewDTO.from_station_telecode != null
					&& bR[bT].queryLeftNewDTO.from_station_telecode == bR[bT].queryLeftNewDTO.start_station_telecode) {
			...
			if (bR[bT].queryLeftNewDTO.to_station_telecode != null
					&& bR[bT].queryLeftNewDTO.to_station_telecode == bR[bT].queryLeftNewDTO.end_station_telecode) {
			...
			bU.push("</strong>");
			bU.push("<span>");
			bU.push(changeArriveDate(bR[bT].queryLeftNewDTO.start_time,
					bR[bT].queryLeftNewDTO.lishi));
			bU.push("到达</span>");
			bU.push("</div>");
			bU.push("</div>");
			bU.push("</td>");
			bD(bU, bR[bT].queryLeftNewDTO.swz_num, "SWZ_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "91");
			bD(bU, bR[bT].queryLeftNewDTO.tz_num, "TZ_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "P1");
			bD(bU, bR[bT].queryLeftNewDTO.zy_num, "ZY_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "M1");
			bD(bU, bR[bT].queryLeftNewDTO.ze_num, "ZE_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "O1");
			bD(bU, bR[bT].queryLeftNewDTO.gr_num, "GR_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "61");
			bD(bU, bR[bT].queryLeftNewDTO.rw_num, "RW_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "41");
			bD(bU, bR[bT].queryLeftNewDTO.yw_num, "YW_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "31");
			bD(bU, bR[bT].queryLeftNewDTO.rz_num, "RZ_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "21");
			bD(bU, bR[bT].queryLeftNewDTO.yz_num, "YZ_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "11");
			bD(bU, bR[bT].queryLeftNewDTO.wz_num, "WZ_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "W1");
			bD(bU, bR[bT].queryLeftNewDTO.qt_num, "QT_",
					bR[bT].queryLeftNewDTO.train_no,
					bR[bT].queryLeftNewDTO.yp_ex, "");
			if ("Y" == bR[bT].queryLeftNewDTO.canWebBuy) {
				bU
						.push(' <td align="center" width="80" class="no-br"><a href="javascript:" class="btn72" onclick="submitOrderRequest(\'');
	...
	function bD(bW, bV, bU, bT, bX, bS) {
		...
		if ("有" == bV) {
			if (bU == "SWZ_" || bU == "TZ_") {
				...
				bW.push('">');
				if (bR) {
					bW.push('<div class="sale" title="本席别票价打折">' + bV
			...
			if (isNum(bV) > 0) {
				if (bU == "SWZ_" || bU == "TZ_" || bU == "ZY_" || bU == "ZE_") {
					bW
							.push('<td width="46" align="center" style="cursor: pointer;" class="t-num" onclick="showTicketPrice(this)" id="');
					bW.push(bU);
					bW.push(bT);
					bW.push('">');
					bW.push("<div>");
	...
	function h(bS, bR) {
		...
		for (i = 0; i < bS.length; i++) {
			if (bS[i][0] == bR) {
				bS[i][1] = $("#".concat($("#".concat(bR)).attr("for"))).is(
						":checked");
				ishaveCheckId = true
			}
		}
	...
	function v(bS) {
		var bR = new Date();
		var bT = bS.split("-");
		bR.setFullYear(parseInt(bT[0]), parseInt(bT[1] - 1, 10), parseInt(
				bT[2], 10));
		if (bT.length >= 6) {
	...
	Date.prototype.format = function(bS) {
		...
		if (/(y+)/.test(bS)) {
			bS = bS.replace(RegExp.$1, (this.getFullYear() + "")
					.substr(4 - RegExp.$1.length))
		}
		for ( var bR in bT) {
			if (new RegExp("(" + bR + ")").test(bS)) {
				bS = bS.replace(RegExp.$1, RegExp.$1.length == 1 ? bT[bR]
						: ("00" + bT[bR]).substr(("" + bT[bR]).length))
			}
		}
		return bS
	};
		...
		for ( var bR = 1; bR <= 20; bR++) {
			...
			if (bT == bS) {
				...
				$("#date_range>ul>li:nth-child(1)").children()
						.addClass("first");
				bt = $("#date_range>ul>li:nth-child(" + bR + ")").children(
						"span:first-child").text();
				break
			}
		}
	...
	function a4() {
		...
		$("#fromStationText").unbind("focus").unbind("click").attr("readonly",
				"true");
		$("#toStationText").unbind("focus").unbind("click").attr("readonly",
				"true");
		$("#fromStationText").removeClass().addClass("inp-txt");
		$("#toStationText").removeClass().addClass("inp-txt");
		$("#change_station").unbind();
	...
	function bk(bS, bY) {
		...
		if (bR) {
			var bX = "";
			if ($.trim($("#fromStation").val()) == $
					.trim($("#toStation").val())
					|| $.trim($("#fromStationText").val()) == $.trim($(
							"#toStationText").val())) {
				bX = "出发地和目的地不能相同";
				b(bX);
				return false
		...
		$("#query_ticket")
				.click(
						function(bV) {
							...
							var bS = train_tour_flag == "fc" ? $.trim($(
									"#back_train_date").val()) : $.trim($(
									"#train_date").val());
							var bR = bk(bS, bW);
							if (!bR) {
	...
	function R() {
		$("#date_range>ul>li").each(
				function(bU) {
					...
					var bT = $(
							"#date_range>ul>li:nth-child(" + (bU + 1)
									+ ")>span[class=hide]").text().substring(0,
							5)
							+ aZ(bR);
					$(
							"#date_range>ul>li:nth-child(" + (bU + 1)
									+ ")>span[class=hide]").text(bT)
				});
		if (index_train_date == null) {
			...
			$("#date_range>ul>li:nth-child(1)").children("span:first-child")
					.addClass("hide");
			bt = $("#date_range>ul>li:nth-child(1)").children(
					"span:first-child").text()
		}
		aX()
	}
	...
	function J(bT, bS) {
		...
		for ( var bR = 0; bR < bS.length; bR++) {
			...
			if (bS[bR] == "QT") {
				if (bT.queryLeftNewDTO.station_train_code.substring(0, 1) != "G"
						&& bT.queryLeftNewDTO.station_train_code
								.substring(0, 1) != "D"
						&& bT.queryLeftNewDTO.station_train_code
								.substring(0, 1) != "C"
						&& bT.queryLeftNewDTO.station_train_code
			...
			if (bS[bR] == "G") {
				if (bT.queryLeftNewDTO.station_train_code.substring(0, 1) == "C"
						|| bT.queryLeftNewDTO.station_train_code
								.substring(0, 1) == "G") {
					return true
				}
	...
	function az(bT, bR) {
		...
		for ( var bS = 0; bS < bR.length; bS++) {
			if (bR[bS] == "SWZ") {
				if (bT.queryLeftNewDTO.swz_num != "--"
						&& bT.queryLeftNewDTO.swz_num != "无") {
					ao.push("SWZ");
					return true
				}
			}
			if (bR[bS] == "TZ") {
				if (bT.queryLeftNewDTO.tz_num != "--"
						&& bT.queryLeftNewDTO.tz_num != "无") {
					ao.push("TZ");
					return true
				}
			}
			if (bR[bS] == "ZY") {
				if (bT.queryLeftNewDTO.zy_num != "--"
						&& bT.queryLeftNewDTO.zy_num != "无") {
					ao.push("ZY");
					return true
				}
			}
			if (bR[bS] == "ZE") {
				if (bT.queryLeftNewDTO.ze_num != "--"
						&& bT.queryLeftNewDTO.ze_num != "无") {
					ao.push("ZE");
					return true
				}
			}
			if (bR[bS] == "GR") {
				if (bT.queryLeftNewDTO.gr_num != "--"
						&& bT.queryLeftNewDTO.gr_num != "无") {
					ao.push("GR");
					return true
				}
			}
			if (bR[bS] == "RW") {
				if (bT.queryLeftNewDTO.rw_num != "--"
						&& bT.queryLeftNewDTO.rw_num != "无") {
					ao.push("RW");
					return true
				}
			}
			if (bR[bS] == "YW") {
				if (bT.queryLeftNewDTO.yw_num != "--"
						&& bT.queryLeftNewDTO.yw_num != "无") {
					ao.push("YW");
					return true
				}
			}
			if (bR[bS] == "RZ") {
				if (bT.queryLeftNewDTO.rz_num != "--"
						&& bT.queryLeftNewDTO.rz_num != "无") {
					ao.push("RZ");
					return true
				}
			}
			if (bR[bS] == "YZ") {
				if (bT.queryLeftNewDTO.yz_num != "--"
						&& bT.queryLeftNewDTO.yz_num != "无") {
					ao.push("YZ");
					return true
				}
			}
			if (bR[bS] == "WZ") {
				if (bT.queryLeftNewDTO.wz_num != "--"
						&& bT.queryLeftNewDTO.wz_num != "无") {
					ao.push("WZ");
					return true
				}
			}
	...
	function q(bS, bR) {
		...
		for ( var bT = 0; bT < bR.length; bT++) {
			if (bR[bT].toLowerCase() == bS.queryLeftNewDTO.station_train_code
					.toLowerCase()) {
				return true
			}
		}
	...
	function aS() {
		...
		var bT = aO;
		var bX = [];
		if (bS.length > 0 || bY.length > 0
				|| filteredTrainArriveTime.length > 0 || bs.length > 0
				|| bU.length > 0 || bW.length > 0 || aj.getComboText() != ""
					...
					if (document.getElementById("autoSubmit").checked) {
						...
						for ( var b1 = 0; b1 < passengerChecked.length; b1++) {
							...
							for ( var b2 = 0; b2 < bV.length; b2++) {
								var bS = bV[b2];
								if (bY.passenger_name == bS.passenger_name
										&& bY.passenger_id_type_code == bS.passenger_id_type_code
										&& bY.passenger_id_no == bS.passenger_id_no) {
									bU = true;
									break
								}
							}
						...
						if (b7[0] == 1 || b7[0] == 2) {
							...
							var b4 = D.secretStr;
							j(b7);
							var b3 = checkusermdId != undefined ? "&_json_att="
									...
									.ajax({
										...
										success : function(ca) {
											if (ca.status) {
												...
												if (ca.data
														&& undefined != ca.data.result
														&& typeof (ca.data.result) != "undefined") {
	...
	function bw() {
		...
		box = dhtmlx.createWin({
			...
			checkConfirm : function() {
				...
				if ("" == randCode) {
					bR = false;
					$("#randCode2").removeClass("inptxt w100").addClass(
							"inptxt w100 error");
					$("#c_error2").html(login_messages.randCodeEmpty).attr(
							"class", "error").css("margin-left", "60px");
					$("#randCode2").focus()
				} else {
					if (!/^[a-zA-Z0-9]+$/.test(randCode)) {
						...
						$("#randCode2").removeClass("inptxt w100").addClass(
								"inptxt w100 error");
						$("#c_error2").html(login_messages.randCodeFormat)
						...
						if (randCode.length != 4) {
							bR = false;
							$("#randCode2").removeClass("inptxt w100")
									.addClass("inptxt w100 error");
							$("#c_error2").html(login_messages.randCodeLentgh)
									.attr("class", "error").css("margin-left",
											"60px");
							$("#randCode2").focus()
						}
				...
				$.ajax({
					...
					success : function(bS) {
						if (bS.data == "N") {
							...
							bR = true;
							$("#randCode2").removeClass("inptxt w100 error")
	...
	function j(b2) {
		...
		for ( var bY = 0; bY < bW.length; bY++) {
			...
			for ( var bZ = 0; bZ < b0; bZ++) {
				...
				tickets_info.push(new aP("sdAdd_" + Z(), bR, bS, bX, bT,
						passengerChecked[bV].passenger_name,
						passengerChecked[bV].passenger_id_type_code,
						passengerChecked[bV].passenger_id_type_name,
						passengerChecked[bV].passenger_id_no,
						passengerChecked[bV].mobile_no));
				bV++
			}
		}
	}
	...
	getpassengerTicketsForAutoSubmit = function() {
		...
		for ( var bS = 0; bS < tickets_info.length; bS++) {
			...
			var bU = bT
					+ ",0,"
					+ tickets_info[bS].ticket_type
					+ ","
					+ tickets_info[bS].name
					+ ","
					+ tickets_info[bS].id_type
					+ ","
					+ tickets_info[bS].id_no
					+ ","
					+ (tickets_info[bS].phone_no == null ? ""
							: tickets_info[bS].phone_no) + ",N";
			bR += bU + "_"
		}
	...
	getOldPassengersForAutoSubmit = function() {
		...
		for ( var bS = 0; bS < passengerChecked.length; bS++) {
			var bR = passengerChecked[bS].passenger_name + ","
					+ passengerChecked[bS].passenger_id_type_code + ","
					+ passengerChecked[bS].passenger_id_no + ","
					+ passengerChecked[bS].passenger_type;
			bT += bR + "_"
		}
		return bT
	};
					...
					.ajax({
						...
						success : function(bZ) {
							if (bZ.status) {
								...
								b0 = a6(bZ.data.ticket,
										tickets_info[0].seat_type).split(",");
								bW = "本次列车，剩余"
								...
								if (bZ.data.op_2 == "true") {
									...
									if (bZ.data.countT > 0) {
										bW += '目前排队人数<font color="red">'
												+ bZ.data.countT + "</font>人，";
			...
			success : function(bR) {
				if (bR.status) {
					if (bR.data.submitStatus) {
						...
						S("出票失败!", false, "原因： " + bR.data.errMsg
	...
	function K() {
		...
		S = function(b1, bY, bV, bU, bX) {
			var b0 = '<div class="tit">'
					+ (bY ? '<span class="colorC">' + b1 + "</span>" : b1)
					+ "</div>";
			var bW = "<P>" + bV + "</p>";
			var bZ = bY ? '<p>请点击[<a href="'
					+ ctx
		...
		function bR(bY) {
			...
			if ("checkticketinfo_id" == bY) {
				...
				if (bW.to_station_telecode == ticket_submit_order.special_areas.lso
						|| bW.to_station_telecode == ticket_submit_order.special_areas.dao
						|| bW.to_station_telecode == ticket_submit_order.special_areas.ado
						|| bW.to_station_telecode == ticket_submit_order.special_areas.nqo
						|| bW.to_station_telecode == ticket_submit_order.special_areas.tho) {
					if (bS()) {
			...
			bV.setDimension($("#content_" + bY).width(), $("#content_" + bY)
					.height() + 10);
		...
		function bS() {
			for ( var bV = 0; bV < limit_tickets.length; bV++) {
				...
				if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.gc)
						&& bU.save_status != ""
						&& bU.id_type == ticket_submit_order.passenger_card_type.passport) {
					return true
				} else {
	...
	function ap(bR) {
		...
		var bV = "日一二三四五六";
		return bU.concat(bT).concat("&nbsp;&nbsp;").concat("周").concat(
				bV.charAt(bS.getDay()))
	}
	showTicketPrice = function(bV) {
		var bY = $(bV).parent("tr").children("td").children("div").children(
			...
			$.ajax({
				...
				success : function(b0) {
					if (b0.status) {
						...
						$("#ticket_" + bW + "_train>span>b").attr("title",
								"收起票价");
						if (bS == "--") {
							b0.data.MIN = ""
						}
						...
						$("#price_" + bW).html(
								$.render.priceTableTemplate(b0.data));
						$("#price_" + bW).show();
						if (b0.data.PM != "--") {
	...
	checkHover = function(bS, bR) {
		if (getEvent(bS).type == "mouseover") {
			return !$.contains(bR, getEvent(bS).relatedTarget
					|| getEvent(bS).fromElement)
					&& !((getEvent(bS).relatedTarget || getEvent(bS).fromElement) === bR)
		} else {
			return !$.contains(bR, getEvent(bS).relatedTarget
					|| getEvent(bS).toElement)
					&& !((getEvent(bS).relatedTarget || getEvent(bS).toElement) === bR)
		}
	};
						...
						if (bT.data.flag) {
							...
							if (r == "0X00") {
								...
								for (i = (studentComPerArr.length - 1); i >= 0; i = i - 2) {
									if (v(studentComPerArr[i - 1]) <= v(bV)
											&& v(studentComPerArr[i]) >= v(bV)) {
										bU = true;
										break
									}
								}
	...
	function L(b4, bX) {
		...
		if (train_tour_flag == "fc") {
			bR = "fc";
			var bU = bX.split(":");
			var bT = $("#back_train_date").val() + "-" + bU[0] + "-" + bU[1]
					+ "-00";
			try {
				if (roundReferTime) {
		...
		if ("undefined" == typeof (submitForm)) {
			var bV = "secretStr=" + b4 + "&train_date="
					+ $("#train_date").val() + "&back_train_date="
					+ $("#back_train_date").val() + "&tour_flag=" + bR
					+ "&purpose_codes=" + bO() + "&query_from_station_name="
			...
			var bW = b3[1].split(",-,")[0];
			var b0 = b3[1].split(",-,")[1];
			var bV = escape(bY) + "=" + escape(b1) + "&" + bW + "=" + b0
					+ "&secretStr=" + b4 + "&train_date="
					+ $("#train_date").val() + "&back_train_date="
					+ $("#back_train_date").val() + "&tour_flag=" + bR
					+ "&purpose_codes=" + bO() + "&query_from_station_name="
					+ $("#fromStationText").val() + "&query_to_station_name="
		...
		var b2 = checkusermdId != undefined ? "&_json_att="
				+ encodeURIComponent(checkusermdId) : "";
		$
				.ajax({
					type : "post",
					...
					success : function(b5) {
						if (b5.status) {
							if (train_tour_flag != null) {
								if (train_tour_flag == "fc") {
									otsRedirect("post", ctx
											+ "confirmPassenger/initFc", {});
									return
								}
								if (train_tour_flag == "gc") {
									otsRedirect("post", ctx
								...
								if (!bU.test(b0)) {
									...
									if (bW.test(b0) && b0.length > 4) {
										...
										if (b0.length < 2) {
											...
											var b8 = $.trim($("#train_date")
													.val());
											var b4 = bV + bY + b8;
											var b5 = b8.split("-");
											var b1 = new Date();
											b1.setFullYear(b5[0], b5[1] - 1,
													b5[2]);
											b1.setHours(8, 0, 0);
											var bX = new Date();
											var b7 = "";
																...
																if (ca.status) {
																	if (ca.data == null
																		...
																		for ( var cb = 0; cb < cc.length; cb++) {
																			bS
																					.push(cc[cb].queryLeftNewDTO.station_train_code
																							.toUpperCase())
																		}
																		A = bS
																	}
				...
				renderPassenger : function() {
					if (passengerAll) {
						...
						for ( var bV = 0; bV < b2; bV++) {
							...
							if (bS != "" && bS != "输入乘客姓名") {
								if (bT.passenger_name.indexOf(bS) > -1
										|| (bT.first_letter && bT.first_letter
												.toUpperCase().indexOf(
														bS.toUpperCase()) > -1)) {
									bX++;
									var bU = "";
									...
									bZ[bV] = "<li " + bY + "><input " + bU
											+ " type='checkbox' " + bW
											+ " name='" + bT.passenger_type
											+ "' class='check' />"
											+ bT.passenger_name + "(" + bR
											+ ")(" + bT.passenger_id_no
											+ ")</li>"
								}
							} else {
								...
								bZ[bV] = "<li " + bY + "><input  " + bU
										+ " type='checkbox' " + bW + " name='"
										+ bT.passenger_type
										+ "' class='check' />"
										+ bT.passenger_name + "(" + bR + ")("
										+ bT.passenger_id_no + ")</li>"
							}
						}
						var b1 = 100;
						var b0 = 0;
				...
				pHasInSelected : function(bU) {
					...
					if (bS > 0) {
						for ( var bR = 0; bR < bS; bR++) {
							...
							if (bT.passenger_name == bU.passenger_name
									&& bT.passenger_id_type_code == bU.passenger_id_type_code
									&& bT.passenger_id_no == bU.passenger_id_no) {
								return true
							}
						}
					}
	...
	function af() {
		...
		var bR = $("#fromStationNameTemplate").html().replace("<!--", "")
				.replace("-->", "");
		$.templates({
			stationNameTableTemplate : bR
		});
		var bR = $("#toStationNameTemplate").html().replace("<!--", "")
				.replace("-->", "");
		$.templates({
	...
	function bi(bS) {
		...
		$(bS).each(
				function() {
					bR.push([ this.queryLeftNewDTO.station_train_code,
	...
	function w(bU) {
		...
		if ("" == bU || bU == null || bU == "用户名／邮箱" || bU == "admin") {
			...
			$("#username").removeClass("inptxt w200").addClass(
					"inptxt w200 error");
			$("#c_error").html(login_messages.userNameEmpty).attr("class",
			...
			if (!bS.test(bU) && !bT.test(bU)) {
				...
				$("#username").removeClass("inptxt w200").addClass(
						"inptxt w200 error");
				$("#c_error").html(login_messages.userNameFormat).attr("class",
						"error").css("margin-left", "60px")
			}
		}
		if (bR) {
	...
	function bc(bS) {
		...
		if ("" == bS || bS == null) {
			...
			$("#password").removeClass("inptxt w200").addClass(
					"inptxt w200 error");
			$("#c_error").html(login_messages.passwordEmpty).attr("class",
					"error").css("margin-left", "60px")
		} else {
			if (bS.length < 6) {
				bR = false;
				...
				$("#password").removeClass("inptxt w200").addClass(
						"inptxt w200 error");
				$("#c_error").html(login_messages.passwordLength).attr("class",
						"error").css("margin-left", "60px")
			}
		}
		if (bR) {
	...
	function aW(bS) {
		...
		if ("" == bS) {
			bR = false;
			ay();
			$("#randCode").removeClass("inptxt w100").addClass(
					"inptxt w100 error");
			$("#c_error").html(login_messages.randCodeEmpty).attr("class",
					"error").css("margin-left", "60px")
		} else {
			if (!/^[a-zA-Z0-9]+$/.test(bS)) {
				...
				$("#randCode").removeClass("inptxt w100").addClass(
						"inptxt w100 error");
				$("#c_error").html(login_messages.randCodeFormat).attr("class",
						"error").css("margin-left", "60px")
			} else {
				if (bS.length != 4) {
					bR = false;
					...
					$("#randCode").removeClass("inptxt w100").addClass(
							"inptxt w100 error");
					$("#c_error").html(login_messages.randCodeLentgh).attr(
							"class", "error").css("margin-left", "60px")
				} else {
					if (!x(bS)) {
						bR = false;
						...
						$("#randCode").removeClass("inptxt w100").addClass(
								"inptxt w100 error");
						$("#c_error").html(login_messages.randCodeError).attr(
								"class", "error").css("margin-left", "60px")
					}
				}
			}
														...
														if (bU.data.status) {
															...
															if (bU.data.loginFail != null) {
																if (bU.data.loginFail == "登录名不存在!") {
																	...
																	if (bU.data.loginFail == "验证码不正确！"
																			&& bU.data.loginFail != "登录名不存在!") {
																		ay();
																		$(
																				"#randCode")
																				.removeClass(
															...
															if (bT.data.status) {
																...
																if (bT.data.loginFail != null) {
																	if (bT.data.loginFail == "登录名不存在!") {
																		...
																		if (bT.data.loginFail == "验证码不正确！"
																				&& bT.data.loginFail != "登录名不存在!") {
																			ay();
																			$(
																					"#randCode")
																					.removeClass(
	...
	function aB() {
		...
		if (train_tour_flag != null && train_tour_flag != ""
				&& train_tour_flag == "fc") {
			bS = jQuery.inArray($("#back_train_date").val().substring(5, 10),
					bR)
		} else {
		...
		if (bS != "-1") {
			bS = bS + 1;
			$("#date_range>ul>li").removeClass("sel");
			$("#date_range>ul>li:nth-child(1)").children("span:first")
					.addClass("first");
			$("#date_range>ul>li:nth-child(1)").children("span:last").addClass(
					"first");
			$("#date_range>ul>li:nth-child(" + bS + ")").addClass("sel");
			$("#date_range>ul>li:nth-child(" + bS + ")").children(
					"span:last-child").removeClass();
			$("#date_range>ul>li:nth-child(1)").children().addClass("first");
			$("#date_range>ul>li:nth-child(" + bS + ")").children(
					"span:first-child").addClass("hide");
			$("#date_range>ul>li:nth-child(20)").addClass("end");
			bt = $("#date_range>ul>li:nth-child(" + bS + ")").children(
					"span:first-child").text()
		}
	}
	function bp() {
	...
	function ab() {
		if ($("#sf2").is(":checked")) {
			if (v($("#train_date").val()) > v(init_maxPeriod) - 24 * 60 * 60
					* 1000) {
				a0()
			} else {
				aX()
		...
		if (bR != rqChecked[0]) {
			for ( var bT = 0; bT < rqChecked.length; bT++) {
				if (bR == rqChecked[bT]) {
					...
					$("#date-list input[value=" + rqChecked[0] + "]").prop(
							"checked", false);
